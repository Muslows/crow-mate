...
import { respondToClubInvitation } from "@/lib/actions/affiliation";
...
type ClubInvite = {
  id: string;
  parentTeam: { name: string };
  childTeam: { name: string };
};
...
export function ClubInviteInbox({ invitations }: { invitations: ClubInvite[] }) {
  if (invitations.length === 0) return null;

  return (
    <section id="club-invites" className="flex flex-col gap-3">
            ...
            <Panel>
              ...
              <p className="mt-2 text-lg">
                ...
                <span className="font-semibold uppercase text-cyan-100">
                  {invitation.childTeam.name}
                </span>{" "}
                souhaite rejoindre le club{" "}
                <span className="font-semibold uppercase">
                  {invitation.parentTeam.name}
                </span>
                .
              </p>
